if __name__=='__main__':
    ...
    def an():
        anfr = Frame(AMframe)
        anfr.pack()
        n = Label(anfr, text='Type name of popular cryptocurrency')
        ...
        zat = Button(anfr,text='Search',command = c)
        zat.grid(row=1,column=1)
        def wroc():
            global but5
            but5 = Button(AMframe, text="Media analaysis", command=an)
            but5.pack()
            anfr.destroy()
            root.geometry('300x250')

        but = Button(anfr, text="Back",fg='Red',command=wroc)
    ...
    # but1 = Button(WSframe, text="Wyswietl saldo",fg='Blue',command=saldo, state = DISABLED)
    ...
    but5 = Button(AMframe, text="Media analaysis", command = an)
    # but6 = Button(ATframe, text="Analiza techniczna", command = at, state=DISABLED)
    # but1.pack()
    # but2.pack()
    # but3.pack()
    # but4.pack()
